...
type MeetGreetRow = {
  ...
  dateOfSubmission: string; // yyyy-mm-dd
...
const VISA_TYPES: Array<MeetGreetRow["visaType"] | "All"> = ["All", "Tourist", "Work", "Business"];

const STATUS_OPTIONS: Array<MeetGreetRow["status"] | "All"> = [
  "All",
  "Pending",
  "Approved",
  "Rejected",
...
const PAYMENT_OPTIONS: Array<MeetGreetRow["payment"] | "All"> = ["All", "Paid", "Pending"];

/* ---------- COMPONENT ---------- */

export default function MeetAndGreetDelhi() {
  /* ---------- MOCK DATA ---------- */
  const [rows] = useState<MeetGreetRow[]>([
    {
      id: "MG-3001",
      email: "user1@mail.com",
      contact: "9876543210",
      dateOfArrival: "2026-01-18",
      dateOfSubmission: "2026-01-12",
      visaType: "Tourist",
      submissionCountry: "Russia",
      status: "Pending",
      payment: "Pending",
      createdAt: "Just now",
    },
    {
      ...
      contact: "9999999999",
      dateOfArrival: "2026-02-03",
      dateOfSubmission: "2026-01-25",
      visaType: "Business",
      submissionCountry: "Serbia",
      status: "Approved",
      payment: "Paid",
  ...
  const [submissionCountry, setSubmissionCountry] = useState<string>("All");
    ...
    if (searchText.trim()) {
      ...
      data = data.filter((r) => {
        return (
          ...
          r.submissionCountry.toLowerCase().includes(q)
        );
      });
    }
  ...
  const stats = useMemo(() => {
    ...
    const byCountry =
      submissionCountry !== "All" ? rows.filter((r) => r.submissionCountry === submissionCountry).length : rows.length;

    const byVisaType = visaType !== "All" ? rows.filter((r) => r.visaType === visaType).length : rows.length;

    const byStatus = status !== "All" ? rows.filter((r) => r.status === status).length : rows.length;

    const byPayment = payment !== "All" ? rows.filter((r) => r.payment === payment).length : rows.length;

    return { total, byCountry, byVisaType, byStatus, byPayment };
  }, [rows, submissionCountry, visaType, status, payment]);
  ...
  useEffect(() => {
    setPage(1);
  }, [pageSize, searchText, submissionCountry, visaType, status, payment]);
  ...
  const onEdit = (row: MeetGreetRow) => alert(`Edit: ${row.id} (wire to modal/page)`);
  const onDelete = (row: MeetGreetRow) => alert(`Delete: ${row.id} (wire to confirm + API)`);
    ...
    <div className="space-y-4">
      ...
      <div className="rounded-2xl border border-slate-200 bg-white p-4">
        ...
        <div className="mt-3 grid grid-cols-1 lg:grid-cols-6 gap-2">
          ...
          <div>
            <label className="text-xs font-medium text-slate-600">Submission Country</label>
            <select
              value={submissionCountry}
              onChange={(e) => setSubmissionCountry(e.target.value)}
              className="mt-1 h-10 w-full rounded-xl border border-slate-200 px-3 text-sm outline-none"
            >
              {SUBMISSION_COUNTRIES.map((c) => (
      ...
      <div className="rounded-2xl border border-slate-200 overflow-hidden">
        <div className="bg-teal-900 text-white px-4 py-3 font-semibold flex items-center justify-between">
          <span>Meet &amp; Greet (Delhi) Enquiries</span>
          ...
          <table className="w-full text-sm">
            <thead className="bg-slate-50 text-slate-600">
              <tr>
                ...
                <th className="text-left px-4 py-3 font-medium">Submission Country</th>
              ...
              {pagedRows.length === 0 ? (
                ...
                pagedRows.map((r) => (
                  <tr key={r.id} className="border-t border-slate-100">
                    ...
                    <td className="px-4 py-3 text-slate-900">{r.dateOfSubmission}</td>
