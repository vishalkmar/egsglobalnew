...
export default function AssistantAppointment() {
  ...
  const filtered = useMemo(() => {
    ...
    if (country !== "All") data = data.filter((r) => r.submissionCountry === country);
  ...
  const stats = useMemo(() => ({
    total: rows.length,
    byCountry: country !== "All" ? rows.filter((r) => r.submissionCountry === country).length : rows.length,
    byVisaType: visaType !== "All" ? rows.filter((r) => r.visaType === visaType).length : rows.length,
    byStatus: status !== "All" ? rows.filter((r) => (r.status || "Pending") === status).length : rows.length,
  ...
  const updateField = async (row: ApiRow, updates: Record<string, string>) => {
    try {
      const res = await fetch(`${API_BASE}/assistant/assistant-appointment/enquiry/${row.id}`, {
        method: "PATCH",
  ...
  return (
    <div className="space-y-4">
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-4"><div className="text-2xl font-semibold text-slate-900">{stats.byStatus}</div><div className="mt-0.5 text-xs text-slate-600">Status wise</div></div>
      </div>
      ...
      <div className="rounded-2xl border border-slate-200 bg-white p-4">
        ...
        <div className="mt-3 grid grid-cols-1 lg:grid-cols-6 gap-2">
          <div className="lg:col-span-2">
            ...
            <div className="mt-1 flex items-center gap-2 rounded-xl border border-slate-200 bg-white px-3 h-10"><Search className="h-4 w-4 text-slate-500" /><input value={searchText} onChange={(e) => setSearchText(e.target.value)} placeholder="Search by name, email, phone..." className="w-full outline-none text-sm" /></div>
          </div>
          <div><label className="text-xs font-medium text-slate-600">Country</label><select className="mt-1 h-10 w-full rounded-xl border border-slate-200 px-3 text-sm outline-none" value={country} onChange={(e) => setCountry(e.target.value)}>{COUNTRIES.map((c) => <option key={c} value={c}>{c}</option>)}</select></div>
          <div><label className="text-xs font-medium text-slate-600">Visa Type</label><select className="mt-1 h-10 w-full rounded-xl border border-slate-200 px-3 text-sm outline-none" value={visaType} onChange={(e) => setVisaType(e.target.value)}>{VISA_TYPES.map((v) => <option key={v} value={v}>{v}</option>)}</select></div>
          <div><label className="text-xs font-medium text-slate-600">Status</label><select className="mt-1 h-10 w-full rounded-xl border border-slate-200 px-3 text-sm outline-none" value={status} onChange={(e) => setStatus(e.target.value)}>{STATUS_OPTIONS.map((s) => <option key={s} value={s}>{s}</option>)}</select></div>
          <div><label className="text-xs font-medium text-slate-600">Payment</label><select className="mt-1 h-10 w-full rounded-xl border border-slate-200 px-3 text-sm outline-none" value={payment} onChange={(e) => setPayment(e.target.value)}>{PAYMENT_OPTIONS.map((p) => <option key={p} value={p}>{p}</option>)}</select></div>
        </div>
      </div>

      <div className="rounded-2xl border border-slate-200 overflow-hidden">
        <div className="bg-[#294d6b] text-white px-4 py-3 font-semibold flex items-center justify-between"><span>Assistant & Appointment Requests</span><span className="text-sm font-semibold opacity-90">Showing: {filtered.length}</span></div>
